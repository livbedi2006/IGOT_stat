...
import { ShieldCheck, UserCheck, Bell, ChevronDown } from "lucide-react";

export function Header({ currentRole, onRoleChange, learner }) {
  return (
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-3 pl-2 border-l border-slate-700/60">
          <div className="text-right hidden sm:block">
            <div className="text-xs font-semibold text-white leading-tight">{learner?.name || "Livjot Singh Bedi"}</div>
            <div className="text-[11px] text-statwise-pale leading-tight">{learner?.designation || "Junior Statistical Officer"}</div>
          </div>
          <div className="w-8 h-8 rounded-full bg-statwise-blue text-white font-semibold text-xs flex items-center justify-center border border-white/20 shadow-sm">
            {learner?.name ? learner.name.charAt(0) : "L"}
          </div>
        </div>
      </div>
    </header>
